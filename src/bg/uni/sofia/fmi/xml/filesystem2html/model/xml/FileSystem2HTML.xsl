...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="FileNodeTemplate" match="FileNode">
        <li class="file">
            <xsl:value-of select="@Name"/> - 
            <b>Size</b> = <xsl:value-of select="@Size"/> Bytes;
            <b>LastDateChange</b> = <xsl:value-of select="@LastDateChanged"/>;
            <b>Executable</b> = <xsl:value-of select="@Executable"/>;
            <b>Hidden</b> = <xsl:value-of select="@Hidden"/>;
            <b>Readable</b> = <xsl:value-of select="@Readable"/>;
            <b>Writable</b> = <xsl:value-of select="@Writable"/>;
        </li>
    </xsl:template>
</xsl:stylesheet>